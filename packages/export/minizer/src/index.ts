...
function renderTopLevelBinding(event: AssignmentEvent): string {
  ...
  const key = segment.key.startsWith('aeon:') ? segment.key : formatBindingKey(segment.key);
  return `${key}${renderAnnotations(event.annotations)}${renderDatatype(event.datatype)}=${renderValue(event.value)}`;
}
